...
if __name__ == "__main__":
    ...
    # top_ten = get_query(client_key, engine_key, "per se")
    ...
    while not (is_complete):
        ...
        res_count = 1
        relevant = [] # Array to hold relevant queries

        # Display and guage results
        for result in top_ten:
            print(f"Result {res_count}")
            print("[")
            print(f" URL: {result['formattedUrl']}")
        ...
        if(precision >= target_precision): # If target precision reached, exit program
            ...
            augment = augment_query(query, relevant)
            print("Indexing Results ....")
